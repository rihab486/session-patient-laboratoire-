...
$requser = $conn ->prepare("SELECT * FROM rdv ");
...
<body>
 ...
 <p  style="display: flex;position: relative; right:10px; text-align:; justify-content:; color: white; font-size: 0.9em;font-weight: 500; margin: 10px; color:  black;">  les demandes d'analyses:</p>
<?php
// --------------------------------
// La requête (exemple) : tous les "objets", classés par "id".
$query = "SELECT * FROM rdv ORDER BY id ASC;";
  try {
...
?>
    ...
    <thead>
        <tr >
            ...
            <th style="color: white; font-size: 0.9em;font-weight: 500;color: black;text-align: center"><strong>adresse Patient</strong></th>
            <th style="color: white; font-size: 0.9em;font-weight: 500;color: black;text-align: center"><strong>nom patient</strong></th>
            <th style="color: white; font-size: 0.9em;font-weight: 500;color: black;text-align: center"><strong>prenom patient</strong></th>
             <th style="color: white; font-size: 0.9em;font-weight: 500;color: black;text-align: center"><strong>type d'analyse</strong></th>
...
<?php
    ...
    foreach ( $rowAll as $row ) 
    {
        ...
        $adresse=$row['adresse'];
        $nomppatient=$row['nompatient'];
        $prenompatient=$row['prenompatient'];
        // DONNÉES À AFFICHER dans chaque cellule de la ligne
?>
<?php 

                         $reqmail2 = $conn->prepare("SELECT * FROM DateRDVconfirme WHERE adresse = ? ");
                         $reqmail2->execute(array($adresse));
                         $userexist2 = $reqmail2->rowCount();
                         if($userexist2 == 1) {
            ...
            <td>
                    <center>
                       <?php 
                        echo "<a style='font-weight: bold; font-size: 0.8em;color: white;height: 38px;padding: 9px  ; '  class='btn btn-primary ' href='modifier.php?adresse=$adresse ' >Fixer ou modifier la date</a> "; 
                        ...
                        <?php
                         ...
                         echo "<a style='font-weight: bold; font-size: 0.8em;color: white;height: 38px;padding: 9px ;' class='btn btn-primary' href='supprimerDemande.php?adresse=$adresse ' ; onclick='return confirm(\"voulez vous supprimer cette demande? \");' >Supprimer la demande</a> "  ; ?>
