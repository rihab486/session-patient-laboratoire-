             ...
             <p style="display: flex; justify-content: center ;color: white;font-size: 0.9em; font-weight: 500;margin: 10px;"><?php echo $_SESSION['nom']; ?></p>
               ...
               <div class="btn-group-vertical  " style="margin: 0px">
                ...
                <a href="envoyerMessagephp.php" class="monBoutton"> Mes messages</a></button>
                  ...
                  <a href="voirResultat.php" class="monBoutton" style="background-color: #2d4d6c;"> mes resultats</a></button>
...
<div class="col-10">
 ...
 <p  style="display: flex;position: relative; right:10px; text-align:; justify-content:; color: white; font-size: 0.9em;font-weight: 500; margin: 10px; color:  black;">  les résultats d'analyses:</p>
    <?php

        

 $query  = $conn->prepare("SELECT * FROM resultatp WHERE nomPatient = ? And prenomPatient = ? And adresse = ? ");

try {
    
    $query ->execute(array($_SESSION['nom'],$_SESSION['prenom'] ,$_SESSION['adresse']));
    $NbreData = $query ->rowCount();    // nombre d'enregistrements (lignes)
    $rowAll = $query ->fetchAll();
  } catch (PDOException $e){ echo 'Erreur SQL : '. $e->getMessage().'<br/>'; die(); }
// --------------------------------
// affichage
  if($NbreData == 0) {
                    echo " <p style='display: flex; text-align:; justify-content:; color: white;font-size: 0.9em;font-weight: 500; margin: 10px; color:  #3a6289;'>pas encore...</p>" ;  }
else 
{
?>
